...
try:
    ...
    sys.path.append('/home/jetson/catkin_ws/image_pkg_scripts/')
    from params import labels_url, plugin_url, engine_url, CONF_THRESH, IOU_THRESHOLD
...
def Cam_RGB_Callback(msg):
    ...
    boundingBoxes.image_header = msg.header
    getImageStatus = True
    ...
    t0 = time.time()
    # 推理
    img, result_boxes, result_scores, result_classid = yolov5_wrapper.infer(img) # 推理返回结果
    fps = 1 / (time.time() - t0)
    num = len(result_classid)
    # 调试
    # print('result_classid',result_classid)
    # print('result_boxes',result_boxes)
    # print('result_scores',result_scores)
    # for i in range(num):
    #     print(f"type: {categories[int(result_classid[i])]}  conf: {result_scores[i]:.3f}\n"
    #             f"bbox:  {result_boxes[i]}\n"
    #             f"center: [{(result_boxes[i][0] + result_boxes[i][2]) / 2:.2f}, {(result_boxes[i][1] + result_boxes[i][3]) / 2:.2f}]"
    #             # ruler:加了个输出高宽
    #             f"height_width: [{(result_boxes[i][3] - result_boxes[i][1]) :.2f}, {(result_boxes[i][2] - result_boxes[i][0]) :.2f}]")
    for i in range(num):
...
if __name__ == "__main__":
    ...
    det_pub = rospy.Publisher('/yolo/detection', BoundingBoxes, queue_size=10)  # 发布检测框信息
    # 订阅机器人相机的图像话题
